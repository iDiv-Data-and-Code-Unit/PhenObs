...
const { admin } = require('./login/userEnvironments.js');
...
test.describe('Home page', () => {
    ...
    test('Navbar | Items', async () => {
        ...
        const navLinkTexts = ['Home', 'Observations', 'Observations', 'Administration', 'Help', 'Sign Out'];
    ...
    test('Home | Jumbotron details', async () => {
        ...
        await expect(page.locator('div h1.display-3 strong')).toHaveText(`${admin.main_garden_name}: ${admin.subgarden_name}`);
        ...
        await expect(page.locator('div h1.display-4.text-right strong')).toHaveText(admin.username);
    });
